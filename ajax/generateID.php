...
function GetID($adjectives, $animals)
{
    ...
    do {
        ...
        if ($result == false) {
            echo "<article class='message is-warning'><div class='message-header'><p> Database error!</p></div></article>";
            break;
        }
        $found = true;
...
{
    $rand = (rand() % 10);
    return ($rand + (rand() % count($array)) + 1) % 1500;
}
function Randomize2($rando, $array)
{

    $final = ($rando + (rand() % count($array)) + 1) % 1500;
    $arr = array();
